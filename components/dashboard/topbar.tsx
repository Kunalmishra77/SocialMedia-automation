...
export function Topbar({ email, unread = 0 }: { email: string; unread?: number }) {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-end gap-3 border-b border-border bg-card/60 px-6 backdrop-blur">
      <Link href="/notifications" className="relative rounded-lg p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" title="Notifications" aria-label={unread > 0 ? `Notifications, ${unread} unread` : 'Notifications'}>
      ...
      <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/40 py-1 pl-1 pr-2">
        ...
        <form action={logoutAction}>
          <button type="submit" className="rounded-md p-1 text-muted-foreground transition-colors hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1" title="Log out" aria-label="Log out">
