...
const STAGE_STYLE: Record<string, string> = {
  created: 'bg-muted/40 text-foreground',
  link_opened: 'bg-sky-500/15 text-sky-400',
  platforms: 'bg-[#ea6a24]/15 text-[#ea6a24]',
...
export default async function OnboardingPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
          ...
          {STAGES.map((s, i) => {
            ...
            return (
              <div key={s.key} className="flex items-center gap-3">
                <span className="w-32 shrink-0 text-xs text-muted-foreground">{i + 1}. {s.label}</span>
                <div className="h-5 flex-1 overflow-hidden rounded bg-muted">
                  <div className="flex h-full items-center rounded bg-[#ea6a24]/70 px-2 text-[11px] font-medium text-white" style={{ width: `${Math.max(pct, c > 0 ? 8 : 0)}%` }}>{c > 0 ? c : ''}</div>
                </div>
              </div>
            )
          })}
      ...
      {/* In-flight clients */}
      <Panel title="In-flight clients" right={<span className="text-xs text-muted-foreground">{r.clients.length} not yet activated</span>}>
        {r.clients.length === 0 ? (
            ...
            {r.clients.map((c) => (
              <div key={c.id} className="flex items-center justify-between gap-3 py-3 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{c.owner_email} · {c.plan ?? 'no plan'} · {c.platforms} platform(s) · started {timeAgo(c.created_at)}</p>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${STAGE_STYLE[c.stage] ?? 'bg-muted/40 text-foreground'}`}>
