...
export default async function MetaMonitorPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <PageHeader
        ...
        action={
          <form action={refreshMetaHealthAction}>
            <button className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm text-foreground hover:bg-muted"><RefreshCw className="h-4 w-4" />Check now</button>
          </form>
        }
      />
      ...
      <Panel title="App rate-limit usage" right={<Gauge className="h-4 w-4 text-muted-foreground" />}>
        {m.appUsage ? (
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">From Meta&apos;s <span className="font-mono">X-App-Usage</span> header (% of hourly limit). Throttling begins near 100%.</p>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">No usage headers yet — connect an account or re-probe. (Meta returns usage only on live API calls.)</p>
        )}
      ...
      <Panel title="Connected accounts (live probe)">
        {m.channels.length === 0 ? (
          <p className="text-sm text-muted-foreground">No Instagram/Facebook accounts connected yet.</p>
        ) : (
              ...
              <div key={i} className="flex items-center justify-between gap-3 py-3 text-sm">
                <div className="min-w-0">
                  <p className="flex items-center gap-2 text-foreground">
                    ...
                    {c.handle && <span className="text-muted-foreground">@{c.handle}</span>}
                    <span className="text-xs text-muted-foreground">{c.workspace}</span>
                  </p>
                  <p className="mt-0.5 text-xs text-muted-foreground">{c.ok ? `operational · ${c.latencyMs}ms` : c.statusLabel}</p>
                </div>
                <div className="shrink-0 text-right text-xs">
                  <span className={c.tokenState === 'expired' ? 'text-red-400' : c.tokenState === 'expiring' ? 'text-amber-400' : 'text-muted-foreground'}>
                    {c.tokenState === 'unknown' ? 'no expiry' : c.tokenState === 'expired' ? 'token expired' : `token ${c.tokenState}`}
                  </span>
                  {c.expiresAt && <p className="text-muted-foreground">{new Date(c.expiresAt).toLocaleDateString()}</p>}
                </div>
              </div>
            ))}
          </div>
