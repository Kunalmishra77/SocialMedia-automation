...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-start justify-center bg-black/60 p-4 pt-[15vh]" onClick={() => setOpen(false)}>
      <div className="w-full max-w-lg overflow-hidden rounded-xl border border-input bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={submit} className="flex items-center gap-2 border-b border-border px-4">
          ...
          />
          <kbd className="rounded border border-input px-1.5 py-0.5 text-[10px] text-muted-foreground">ESC</kbd>
        </form>
        <p className="px-4 py-2.5 text-xs text-muted-foreground">Type at least 2 characters and press Enter.</p>
      </div>
    </div>
  )
}
