  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="divide-y divide-zinc-800">
          {a.providers.map((p) => (
            <div key={p.name} className="flex items-center justify-between py-3 text-sm">
              <span className="flex items-center gap-2 text-zinc-200"><Cpu className="h-4 w-4 text-zinc-600" />{p.name}</span>
              <span className="flex items-center gap-2">
                <StatusDot state={p.configured ? 'operational' : 'down'} />
                <span className={p.configured ? 'text-emerald-400' : 'text-zinc-500'}>{p.configured ? 'Configured' : 'Not set'}</span>
              </span>
            </div>
          ))}
        </div>
        <p className="mt-3 text-xs text-zinc-600">Requests use the default model, then fall back to OpenRouter free models on failure. Token/cost accounting requires provider billing APIs (not yet wired) — event counts shown instead of fabricated costs.</p>
      </Panel>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Panel title="Top consumers · 30 days" right={<Building2 className="h-4 w-4 text-zinc-600" />}>
